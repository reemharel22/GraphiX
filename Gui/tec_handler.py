...
class file_handler(object):
    ...
    def remove_files(this, file_names):
        print "Done clean."
        for f in file_names:
             if ut.file_exists(f):
                 os.remove(f)
    ...
    def get_xy_max_min(self, infilename):
        f = open(infilename, 'r')
        contents = f.readlines()
        contour_edge = self.get_min_max(contents)
        x_min, x_max, y_min, y_max = self.get_xMax_yMax(contents)
        self.close_file(f)
        return ut.calculate_limits_aspect_ratio(x_min, x_max, y_min, y_max)


    def get_x_y_coordinates(self, infilename):
        f = open(infilename, 'r')
        contents = f.readlines()
        nx, ny = gself.et_Nx_Ny(contents)
        start_mesh_data, end_mesh_data = self.get_mesh_data_indices(nx, ny, contents)

        x_coord, y_coord = self.seperate_data(contents[start_mesh_data:end_mesh_data], nx, ny)
        return x_coord, y_coord


    def get_data_table(self, infilename):
        t1 = time.time()
        f = open(infilename, 'r')
        contents = f.readlines()
        nx, ny = self.get_Nx_Ny(contents)
        start_mesh_data, end_mesh_data = self.get_mesh_data_indices(nx, ny, contents)

        x_coord, y_coord = self.seperate_data(contents[start_mesh_data:end_mesh_data], nx, ny)

        start_velocity_data, end_velocity_data = self.get_velocity_start_end_indices(contents)
        u_vel, v_vel = self.seperate_data(contents[start_velocity_data + 1:end_velocity_data], nx, ny)

        self.close_file(f)
        contours = GraphiX.getdata(nx, ny,  int(contents[1].split()[0]), infilename, end_velocity_data + 1)

        contour_lim = self.get_contour_limits(contents[:100], int(contents[1].split()[0]))
        return nx, ny, x_coord, y_coord, u_vel, v_vel, contours, contour_lim


    def get_specific_contour_data(self, infilename, cnt_name):
        f = open(infilename, 'r')
        contents = f.readlines()
        cnt_index = self.get_contour_index(cnt_name, contents[0:100])
        nx, ny = self.get_Nx_Ny(contents)
        start_mesh_data, end_mesh_data = self.get_mesh_data_indices(nx, ny, contents)
        start_contour_data = self.find_start_contour(contents, nx, ny, end_mesh_data)
        end_contour_data = nx * ny + start_contour_data
        return self.get_contour_array(cnt_index, contents[start_contour_data + 1:end_contour_data], nx, ny)


    def get_contour_array(self, index, contents, nx, ny):
        x = np.ndarray([nx * ny])
        k = 0
        for i in contents:
    ...
    def get_contour_index(self, name, contents):
    ...
    def get_contour_limits(self, first_lines, n_cont):
        contours = {}
        cont_offset = 2
    ...
    def seperate_data(self, data, nx, ny):
        x = np.ndarray([nx*ny])
        y = np.ndarray([nx*ny])
    ...
    def seperate_contours(self, data, nx, ny, num_contours):
    ...
    def make_mesh_file(self, infilename, override=True):
        t1 = time.time()
        f = open(infilename, 'r')
        contents = f.readlines()
        nx, ny = self.get_Nx_Ny(contents)
        if not self.check_file_exists(infilename):
            print "No file with the name {0}".format(infilename)
            return ""
        else:
            x_min, x_max, y_min, y_max = self.get_xMax_yMax(contents)
            f_name = self.get_mesh_name(infilename)
            if not self.check_file_exists(f_name) or override:
                start_mesh_data, end_mesh_data = self.get_mesh_data_indices(nx, ny, contents)
                x, y, ind = self.pre_mesh_2d(nx, ny, contents[start_mesh_data:end_mesh_data])
    ...
    def create_mesh_2d_leeor(self, nx, ny, x, y, ind):
        title_data = [None] * 1
        title_data[0] = "#MESH\n"
        ttt = [None] * 1
        ttt[0] = "\n"
        angle_arr = self.angle_array1(nx, ny, x, y, ind)
        radial_arr = self.radial_array1(nx, ny, x, y, ind)

        return title_data + ttt + radial_arr + ttt + angle_arr


    def make_velocity_file(self, infilename, override=True):
        t1 = time.time()
        f = open(infilename, 'r')
        contents = f.readlines()
        ...
        x_min, x_max, y_min, y_max = self.get_xMax_yMax(contents)
        contour_edge = self.get_min_max(contents)
        f_name = self.get_velocity_name(infilename)

        if not self.check_file_exists(f_name) or override:
            cell = (math.sqrt(pow(x_max - x_min, 2) + pow(y_max - y_min, 2)) / (math.sqrt(Nx * Ny)))
            max_velocity = self.get_max_velocity(contents)
            if max_velocity == 0:
                max_velocity = 1
            start_mesh_data, end_mesh_data = self.get_mesh_data_indices(Nx, Ny, contents)
            start_velocity_data, end_velocity_data, new_velocity = \
                self.velocity_segment(Nx, Ny, start_mesh_data, end_mesh_data, contents, max_velocity, cell)
            new_velocity = self.insert_nx_ny(contents) + new_velocity
            new_velocity = self.create_new_segment(new_velocity)
            f1 = self.open_write(f_name, new_velocity)
    ...
    def make_contour_file(self, infilename, override=True):
        t1 = time.time()
        f = open(infilename, 'r')
        contents = f.readlines()
        ...
        Nx, Ny = self.get_Nx_Ny(contents)
        contour_edge = self.get_min_max(contents)
        f_name = self.get_contour_name(infilename)
        if not self.check_file_exists(f_name) or override:
            start_mesh_data, end_mesh_data = self.get_mesh_data_indices(Nx, Ny, contents)
            end_velocity_data = self.find_start_contour(contents, Nx, Ny, end_mesh_data)
            end_contour, new_contour = self.contour_segment(Nx, Ny, contents, start_mesh_data, end_mesh_data, end_velocity_data)
            new_contour = self.insert_nx_ny(contents) + self.max_min_contours(contents) + new_contour
            new_contour = self.create_new_segment(new_contour)
            f1 = self.open_write(f_name, new_contour)
            self.close_file(f1)
    ...
    def make_vof_file(self, infilename, multiplier_x=1, multiplier_y=1, addition_name="", offset=2):
        t1 = time.time()
        f = open(infilename, 'r')
        contents = f.readlines()
        Nx, Ny = self.get_Nx_Ny(contents)
        f_name = self.get_vof_name(infilename + addition_name)
        len_vof = 0
        if not self.check_file_exists(f_name):
            start_mesh_data, end_mesh_data = self.get_mesh_data_indices(Nx, Ny, contents)
            new_vof, len_vof = self.vof_data_segment(contents[start_mesh_data + Nx*Ny: end_mesh_data], Nx, Ny, offset, multiplier_x, multiplier_y)
            new_vof = self.create_new_segment(new_vof)
            f1 = self.open_write(f_name, new_vof)
            self.close_file(f1)

        return f_name, len_vof


    def heatmap_cython_file(self, x_data, y_data, cont_data, f_name, cont_name, c_min, c_max, addition_name="", offset=0):
        if not f_name.__contains__("_contour.gx"):
            f_name = self.get_contour_name(f_name)
    ...
    def contour_cython_file(self, x_data, y_data, cont_data, f_name, cont_name, times, c_min, c_max):
        if not f_name.__contains__("_contour.gx"):
            f_name = self.get_contour_name(f_name)
            if not os.path.exists(f_name):
                print "No file found. Exiting"
        ...
        f_cntr = self.get_contourcython_name(f_name, cont_name, times)
        GraphiX.calc_contour(f_cntr, int(Nx), int(Ny), int(times), float(c_max), float(c_min),
                             cont_data, x_data, y_data)
        return f_cntr


    def get_heatmapcython_name(self, f_name, cont_name):
        f_name = self.create_graphix_tmp(f_name.split("_contour.gx")[0] +
                                    "_" + str(cont_name) + "_" +"heatmap.gx")
        return f_name


    def get_contourcython_name(self, f_name, cont_name, times):
        f_name = self.create_graphix_tmp(f_name + "_" + str(cont_name) + "_" + str(times) + ".gx")
        return f_name


    def get_mesh_name(self, f_name):
        f_name = self.create_graphix_tmp(f_name + "_mesh.gx")
        return f_name


    def get_vof_name(self, f_name):
        f_name = self.create_graphix_tmp(f_name + "_vof.gx")
        return f_name


    def get_contour_name(self, f_name):
        f_name = self.create_graphix_tmp(f_name + "_contour.gx")
        return f_name


    def get_velocity_name(self, f_name):
        f_name = self.create_graphix_tmp(f_name + "_velocity.gx")
        return f_name
    ...
    def create_graphix_tmp(self, name):
    ...
    def insert_nx_ny(self, dataf):
        Nx, Ny = self.get_Nx_Ny(dataf)
        arr = [None] * 2
        arr[0] = "#Nx " + str(Nx) + "\n"
        arr[1] = "#Ny " + str(Ny) + "\n"
        return arr
    ...
    def max_min_contours(self, dataf):
    ...
    def read_file(self, infilename):
        ...
        self.insert_hashtags_to_start(start_mesh_data, contents)
        ...
        # mesh data segment
        new_mesh = self.mesh_data_segment(Nx, Ny, contents[start_mesh_data: end_mesh_data])
        t4 = time.time()
        ...
        new_start = self.start_segment(Nx, Ny, contents[0:start_mesh_data], start_mesh_data, end_mesh_data)

        contents = self.create_new_content(new_start,
                          new_mesh, new_velocity, new_contour, end_contour, contents)
        t5 = time.time()
        print "time took for create new content +start segment: " + str(t5 - t4)
        f1 = self.open_write(infilename + ".gnu", contents)
        t6 = time.time()
        print "time took for open write: " + str(t6 - t5)

        self.close_files(f1, f)
        t7 = time.time()
        print "time took for close: " + str(t7 - t6)
        return x_min, x_max, y_min, y_max, Nx, Ny


    def find_start_contour(self, dataf, Nx, Ny, end_mesh):
    ...
    def start_segment(self, Nx, Ny, contents, start_mesh_data, end_mesh_data):
    ...
    def contour_segment(self, Nx, Ny, contents, start_mesh, end_mesh, end_velocity_data):
        ...
        new_contour = self.insert_mesh_data_contour(contour_part, mesh_part, title_part)

        return end_contour_data, new_contour


    def insert_mesh_data_contour(self, data_contour, data_mesh, title):
    ...
    def get_xMax_yMax(self, content):
        st = content[1].split()
        return float(st[8]), float(st[9]), float(st[6]), float(st[7])


    def get_max_velocity(self, contents):
        st = contents[1].split()
        return float(st[10])


    # get the mesh'es indices (start & end)
    def get_mesh_data_indices(self, Nx, Ny, contents):
        start_mesh_data = self.get_start_index_mesh_data(contents)
        end_mesh_data = self.get_end_index_mesh_data(Nx, Ny, start_mesh_data, contents)
        return start_mesh_data, end_mesh_data


    # deals with the velocity part in the output file
    def velocity_segment(self, Nx, Ny, start_mesh_data, end_mesh_data, contents, v_max, cell):
        start_velocity_data, end_velocity_data = self.get_velocity_start_end_indices(contents)
        ...
        new_velocity = self.multiply_velocity(velocity_part, mesh_part, title_part, v_max, cell)
        return start_velocity_data, end_velocity_data, new_velocity


    # insert more points for velocity data.
    def multiply_velocity(self, data_velocity, data_mesh, title, v_max, cell):
        for i in range(len(data_velocity)):
            ...
            str_vel = self.normalize_velocity(str_vel,  v_max, cell)
    ...
    def normalize_velocity(self, str_vel, v_max, cell):
    ...
    def mesh_data_segment(self, Nx, Ny, dataf):
        title_data = [None] * 1
        title_data[0] = "#MESH\n"
        ttt = [None] * 1
        ttt[0] = "\n"
        angle_arr = self.angle_array(Nx, Ny, dataf)
        radial_arr = self.radial_array(Nx, Ny, dataf)
    ...
    def pre_mesh_3d(self, nx, ny, nz, contents):
        x = [None] * nx * ny * nz
        y = [None] * nx * ny * nz
        z = [None] * nx * ny * nz
        ind = [None] * nx * ny * nz
    ...
    def pre_mesh_2d(self, nx, ny, contents):
    ...
    def radial_array(self, Nx, Ny, dataf):
        radial_arr = ["0"] * (Nx * Ny)
        #zogi.
    ...
    def angle_array(self, Nx, Ny, dataf):
        angle_arr = ["0"] * (Nx * Ny)
        # even
    ...
    def angle_array1(self, nx, ny, x, y, ind):
        angle_arr = ["0"] * (nx * ny)
        # even
        for i in range(0, nx, 2):
            t = i * ny
    ...
    def radial_array1(self, nx, ny, x, y, ind):
        radial_arr = ["0"] * (nx * ny)
        #zogi.
        for i in range(0, ny, 2):
            for j in range(0, nx):
    ...
    def angle_array3d(self, nx, ny, x, y, z, ind):
        angle_arr = ["0"] * (nx * ny)
        # even
        for i in range(0, nx, 2):
            t = i * ny
    ...
    def radial_array3d(self, nx, ny, x, y,z, ind):
        radial_arr = ["0"] * (nx * ny)
    ...
    def get_velocity_start_end_indices(self, content):
        i = 0
    ...
    def vof_data_segment(self, dataf, nx, ny, offset, mirror_x, mirror_y):
    ...
    def insert_hashtags_to_start(self, start_data, contents):
    ...
    def get_start_index_mesh_data(self, content):
        for i in range(len(content)):
    ...
    def get_index_mesh_3d(self, content, nx, ny, nz):
        start = 0
        end = 0
    ...
    def get_end_index_mesh_data(self, Nx, Ny, start_index, contents):
        i = Nx * Ny - 1
        # no connecting line in the middle of mesh
    ...
    def create_new_content(self, new_start, new_mesh, new_velocity, new_contour, new_end, contents):
        contents = new_start + new_mesh + new_velocity + new_contour + contents[new_end:]
        contents = "".join(contents)
        return contents


    def create_new_segment(self, dataf):
        dataf = "".join(dataf)
        return dataf


    # close the original and new output files
    def close_files(self, f1, f2):
        f1.close()
        f2.close()


    # close the original and new output files
    def close_file(self, f1):
        f1.close()


    # opens a new file .
    def open_write(self, file_name, contents):
        f1 = open(file_name, "w")
        f1.write(contents)
        return f1


    def get_min_max(self, contents):
    ...
    def prepare_contour_array(self, data, cont_num):
        i = 0
    ...
    def get_cmax(self, dataf, cont_name):
        i = 0
    ...
    def read_mass_diag(self, nx, ny, mat_id, list_points, path):
        mat_id = int(mat_id - 1)
